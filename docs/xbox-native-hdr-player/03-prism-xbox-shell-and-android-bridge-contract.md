...
## 3. Xbox `xbox/PrismXbox/` shell — current state

**Directory contents** (excluding `.vs/`, `obj/`, `bin/`, `AppPackages/`): `App.xaml`/`App.xaml.cs`, `MainPage.xaml`/`MainPage.xaml.cs`, `Package.appxmanifest`, `PrismXbox.csproj`/`.csproj.user`/`.sln`, `PrismXbox_TemporaryKey.pfx`/`.cer`, `Properties\AssemblyInfo.cs`/`Default.rd.xml`, `Assets\*` (logos/splash), `www\` (synced build output: `index.html`, `assets/*.js`, `manifest.webmanifest`, `sw.js`).
...
- `coreWebView.SetVirtualHostNameToFolderMapping("prismxbox.local", "www", CoreWebView2HostResourceAccessKind.Allow)` (line 83-84) maps the local `www/` folder to `https://prismxbox.local/`, then navigates `webView.Source` to `https://prismxbox.local/index.html` (lines 31/89).
