...
namespace PrismUwp
{
    ...
    sealed partial class App : Application
    {
        public App()
        {
            ...
            // Remote DevTools - there's no keyboard/mouse on the console itself to drive
            // WebView2's own F12 DevTools window, and this is the only way to see what the
            // page's JS actually observes (console.log, document.activeElement,
            ...
            // --remote-debugging-port=9222 alone (Chromium's usual flag) only binds loopback,
            // which is fine when this shell runs as a local PC debug target
            // (http://localhost:9222/json) but unreachable from a PC over the network to a real
            // Xbox - confirmed empirically, http://<xbox-ip>:9222/json returns nothing there.
            // Xbox instead requires the separate, Device-Portal-tunneled
            // msEdgeDevToolsWdpRemoteDebugging feature, discovered via edge://inspect ->
            // "Connect to a remote Windows device" -> https://<xbox-ip>:11443 (the same
            // Device Portal used for sideloading), not a direct hit on 9222. Both flags are
            // set together so either debugging path works depending on which target this is
            // running on. See Microsoft's own doc:
            // https://learn.microsoft.com/en-us/microsoft-edge/webview2/how-to/remote-debugging-xbox
            Environment.SetEnvironmentVariable(
                "WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS",
                "--remote-debugging-port=9222 --enable-features=msEdgeDevToolsWdpRemoteDebugging");
